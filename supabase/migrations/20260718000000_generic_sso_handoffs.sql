-- Generic, app-scoped, one-time SSO handoffs from MJCC/KpnCompute to any
-- allowlisted external application (e.g. Marquee). Additive alongside
-- lunchvoice_sso_handoffs, which stays as-is for backward compatibility.
...
create index if not exists sso_handoffs_expires_idx
  on public.sso_handoffs (expires_at);

create index if not exists sso_handoffs_user_id_idx
  on public.sso_handoffs (user_id);

create index if not exists sso_handoffs_target_app_idx
  on public.sso_handoffs (target_app);
...
-- Marquee manager access, mirroring how the lioncafe scope was seeded.
