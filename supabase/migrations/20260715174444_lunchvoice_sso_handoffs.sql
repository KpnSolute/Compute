-- One-time, server-only SSO handoffs from MJCC/KpnCompute to Lunchvoice.
...
-- Capture the existing live LionCafe role mapping for fresh environments without
-- overwriting an operator's later role-scope choices.
