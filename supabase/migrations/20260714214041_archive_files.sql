...
create index if not exists archived_files_uploaded_at_idx on public.archived_files (uploaded_at desc);
create index if not exists archived_files_category_idx on public.archived_files (category, uploaded_at desc);
create index if not exists archived_files_sha256_idx on public.archived_files (sha256) where status = 'active';
create index if not exists archived_files_uploaded_by_idx on public.archived_files (uploaded_by, uploaded_at desc);
...
create policy "service role manages archived files" on public.archived_files for all to service_role using (true) with check (true);;
