...
-- Fix 1: dashboard_summary should use the monthly price when available
-- (invoice-applied prices live in monthly_inventory.unit_price)
-- COALESCE(mi.unit_price, ii.unit_price) = use monthly if set, else catalog
...
SELECT
  ...
  ii.sku,
  ii.description,
  -- Use monthly price if set (invoice applied), otherwise catalog price
