...
BEGIN
  ...
  LOOP
    ...
    DO UPDATE SET
      ...
      -- Only reset weekly columns if they are still all 0 (no transactions entered yet).
      -- If real data exists, preserve it and only update the opening balance.
      w1_received = CASE WHEN (monthly_inventory.w1_received + monthly_inventory.w2_received +
                                monthly_inventory.w3_received + monthly_inventory.w4_received +
                                monthly_inventory.w1_issued   + monthly_inventory.w2_issued   +
                                monthly_inventory.w3_issued   + monthly_inventory.w4_issued) = 0
