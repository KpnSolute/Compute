...
-- Fix: week_number CHECK allows 0 for rollover carry-forward entries
ALTER TABLE commit_changes DROP CONSTRAINT IF EXISTS commit_changes_week_number_check;
ALTER TABLE commit_changes ADD CONSTRAINT commit_changes_week_number_check
  CHECK (week_number BETWEEN 0 AND 4);
-- 0 = rollover carry-forward (not a real invoice week)

-- Recreate RPC with corrected week_number
...
BEGIN
  ...
  LOOP
    ...
    -- week_number = 0 marks this as a rollover carry-forward, not an invoice week
