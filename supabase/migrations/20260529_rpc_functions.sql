-- RPC functions for the commit-based inventory system.
--
-- merge_single_staging   — validate + merge one staging entry
-- push_all_staging       — atomically push ALL pending entries as one commit
-- revert_to_commit       — restore inventory to the state captured by a commit
-- cleanup_expired_staging — purge TTL-expired entries
-- Data migration         — copy any existing pending_submissions into staging_entries
--
...
BEGIN
  ...
  -- ── Upsert monthly_inventory row if missing ────────────────────────
  ...
  -- ── Apply field change (absolute set, not incremental) ─────────────
  ...
  -- ── Build snapshot for inventory_versions ──────────────────────────
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'item_id',      mi.item_id,
...
DECLARE
  ...
  v_ym_pair    RECORD;
BEGIN
  -- Guard: at least one pending entry must exist
  ...
  LOOP
    ...
    -- Ensure monthly_inventory row exists
    INSERT INTO monthly_inventory (item_id, month, year, on_hand,
                                   w1_received, w2_received, w3_received, w4_received,
                                   w1_issued,   w2_issued,   w3_issued,   w4_issued)
    VALUES (v_entry.item_id, v_entry.month, v_entry.year, 0,
    ...
    -- Apply field change (absolute set)
    UPDATE monthly_inventory
    SET
      w1_received = CASE WHEN v_entry.week_number = 1 AND v_entry.field = 'received'
                         THEN v_entry.submitted_value ELSE w1_received END,
  ...
  -- ── Create inventory_versions snapshot per unique (month, year) ────
  FOR v_ym_pair IN
  ...
  -- ── Delete all consumed staging entries ────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────
-- revert_to_commit
-- Restores monthly_inventory to the state stored in the snapshot linked
-- to p_target_commit_id.  Creates a new commit documenting the revert.
-- Returns the new (revert) commit_id.
-- ────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Find the inventory_version linked to this commit ───────────────
  ...
  LOOP
    ...
    -- If the row existed before, create a commit_changes for any diff
    IF FOUND THEN
      INSERT INTO commit_changes (commit_id, item_id, month, year, week_number,
                                  field, old_value, new_value, action)
      VALUES (
...
-- Deletes staging entries whose TTL has expired.
...
--
-- Runs only if the pending_submissions table exists.  Idempotent —
-- skips entry_id values already present in staging_entries.
-- ────────────────────────────────────────────────────────────────────────
DO $$
BEGIN
