...
const API = {
  ...
  login: (payload) =>
    API.request('/api/auth/login', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  createItem: (data) =>
    API.request('/api/inventory/items', { method: 'POST', body: JSON.stringify(data) }),
  updateItem: (id, data) =>
    API.request(`/api/inventory/items/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  stageCommit: (data) =>
    API.request('/api/inventory/commits/stage', { method: 'POST', body: JSON.stringify(data) }),
  ...
  rejectStaged: (id, note) =>
    API.request(`/api/inventory/pending/${id}/reject`, {
      method: 'POST',
      body: JSON.stringify({ review_note: note || '' }),
    }),
  deleteStagedEntry: (id) => API.request(`/api/inventory/staging/${id}`, { method: 'DELETE' }),

  pushCommits: (message, branch = 'main', month, year) =>
    API.request('/api/inventory/commits/push', {
      method: 'POST',
      body: JSON.stringify({ message, branch, month, year }),
    }),

  getCommits: (page = 1, perPage = 20) =>
    API.request(`/api/inventory/commits?page=${page}&per_page=${perPage}`),
  ...
  publishMonth: (data) =>
    API.request('/api/inventory/publish', { method: 'POST', body: JSON.stringify(data) }),
  rollover: (data) =>
    API.request('/api/inventory/rollover', { method: 'POST', body: JSON.stringify(data) }),
  saveSnapshot: (data) =>
    API.request('/api/inventory/snapshot', { method: 'POST', body: JSON.stringify(data) }),

  // ── Versions ─────────────────────────────────────────────────────────

  getVersions: (month, year) => API.request(`/api/inventory/versions?month=${month}&year=${year}`),
  createVersion: (data) =>
    API.request('/api/inventory/versions', { method: 'POST', body: JSON.stringify(data) }),
  ...
  parseInvoice: (data) =>
    API.request('/api/inventory/parse-invoice', { method: 'POST', body: JSON.stringify(data) }),
  applyInvoice: (data) =>
    API.request('/api/inventory/apply-invoice', { method: 'POST', body: JSON.stringify(data) }),
  ...
  exportBarcodes: (data) =>
    API.request('/api/inventory/barcodes/export', { method: 'POST', body: JSON.stringify(data) }),
  ...
  updateUser: (id, data) =>
    API.request(`/api/users/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteUser: (id) => API.request(`/api/users/${id}`, { method: 'DELETE' }),
  resetPin: (id, data) =>
    API.request(`/api/users/${id}/pin`, { method: 'PATCH', body: JSON.stringify(data) }),

  // ── Settings  /api/settings/* ────────────────────────────────────────

  getSettings: () => API.request('/api/settings'),
  updateSetting: (key, value) =>
    API.request('/api/settings', { method: 'PATCH', body: JSON.stringify({ key, value }) }),

  // ── GitHub sync  /api/github/* ────────────────────────────────────────

  getGithubStatus: () => API.request('/api/github/status'),
  triggerGithubSync: (month, year) =>
    API.request('/api/github/sync', { method: 'POST', body: JSON.stringify({ month, year }) }),
  ...
  uploadFile: (formData) =>
    fetch('/api/files/upload', { method: 'POST', credentials: 'include', body: formData }).then(
      (r) => r.json(),
    ),
