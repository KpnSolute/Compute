...
const MACHINES = [
    {
        id: "hobart-am15",
        name: "Hobart AM-15 (High-Temp)",
        type: "high" as const,
    },
    {
        id: "jackson-low",
        name: "Jackson Avenger (Low-Temp)",
        type: "low" as const,
    },
];
...
export function MachineLog({ user, period, setPeriod }: PeriodFormProps) {
    ...
    function setR(id: string, f: string, v: string) {
        update((d: any) => ({
            ...d,
            rows: d.rows.map((r: any) => (r.id === id ? { ...r, [f]: v } : r)),
        }));
    }
    function addR() {
        update((d: any) => ({ ...d, rows: [...d.rows, blankRow()] }));
    }
    function delR(id: string) {
        update((d: any) => ({
            ...d,
            rows: d.rows.filter((r: any) => r.id !== id),
        }));
    }
    ...
    return (
        <div>
            ...
            <div className="form-note">
                Record temperatures, flow pressure
                {machine.type === "low" ? " and ppm " : " "}once during each
                meal period. Use a separate log per machine; keep on file for
                one year.
            </div>
            <div className="card" style={{ marginBottom: 14 }}>
                ...
                {canEdit && (
                    <div style={{ padding: "10px 14px" }}>
                        <button className="btn-addrow" onClick={addR}>
                            {I.plus({ style: { width: 13, height: 13 } })} Add
                            reading
                        </button>
                    </div>
                )}
                    ...
                    <table className="data logtbl">
                        <thead>
                            <tr>
                                ...
                                <th colSpan={3} className="grp">
                                    Cooling
                                </th>
                                <th colSpan={4} className="grp rcv">
                                    Reheating
                                </th>
                            ...
                            <tr>
                                <th>Start</th>
                                <th className="r">Temp @2h</th>
                                <th className="r">Final @6h</th>
                        ...
                        <tbody>
                            {rows.map((r: any) => {
                                const t6bad =
                                    r.t6 !== "" && parseFloat(r.t6) > 40;
                                const t2bad =
                                    r.t2 !== "" && parseFloat(r.t2) > 70;
                                return (
                                    <tr key={r.id}>
                                        <td>
                                            {dateCell(
...
export function MealLog({ user }: FormProps) {
    ...
    const rows = data.rows || [];
    function setR(id: string, f: string, v: any) {
        update((d: any) => ({
            ...d,
            rows: d.rows.map((r: any) => (r.id === id ? { ...r, [f]: v } : r)),
        }));
    ...
    const counts = MEAL_COLS.reduce((a: Record<string, number>, c) => {
    ...
    return (
        <div className="fade-in">
            <div className="page-head">
                <div>
                    <h2>Daily Staff / Visitor Meal Log</h2>
                    <div className="ph-sub">
                        All staff and visitors must sign. Dining-hall monitors
                        are not charged for the meal they monitor.
                    </div>
                </div>
            ...
            <div className="meal-summary">
                {MEAL_COLS.map((c) => (
                    <div className="ms-pill" key={c}>
                        <span className="ms-n">{counts[c]}</span>
                        <span className="ms-l">{c}</span>
                    </div>
                ))}
                <div className="ms-pill total">
                    <span className="ms-n">{signedRows.length}</span>
                    <span className="ms-l">Signed in</span>
                </div>
                <div className="ms-pill">
                    <span className="ms-n">{paidCount}</span>
                    <span className="ms-l">Paid tickets</span>
                </div>
                <div className="ms-pill">
                    <span className="ms-n">{compCount}</span>
                    <span className="ms-l">Complimentary</span>
                </div>
            </div>
            <div className="card" style={{ marginBottom: 14 }}>
                <div className="tbl-wrap">
            ...
            <div className="card" style={{ marginBottom: 14 }}>
                <div className="card-body monitor-counts">
                    <label className="ft-field">
                        <span>Monitors used this meal</span>
                        <input
                            className="ipt sel"
                            type="number"
                            min={0}
                            ...
                            onChange={(e) =>
                                update((d: any) => ({
                                    ...d,
                                    monitors: e.target.value,
                                }))
                            }
                        />
                    </label>
                    <label className="ft-field">
                        <span>Staff / visitors eating this meal</span>
                        <input
                            className="ipt sel"
                            type="number"
                            min={0}
...
export function InspectionSheet({ user }: FormProps) {
    ...
    function setRating(i: number, v: string) {
        update((d: any) => ({ ...d, ratings: { ...d.ratings, [i]: v } }));
    }
    const done = Object.keys(ratings).length,
        total = INSPECTION_Q.length;
    ...
    return (
        <div className="fade-in">
            <div className="page-head">
                <div>
                    <h2>Food Services Inspection Sheet</h2>
                    <div className="ph-sub">
                        Rate each category. Explain any poor rating below.
                    </div>
                </div>
                <div className="ph-actions">
                    <span className="saved-chip">
            ...
            <div className="card" style={{ marginBottom: 14 }}>
                <div className="card-head">
                    <h3>Comments &amp; recommendations</h3>
                </div>
...
export function FoodRequest({ user }: FormProps) {
    ...
    const FIELD_HINTS: Record<string, string> = {
        originator: "First + last name",
        dept: "Department or program requesting support",
        ext: "Center extension",
        eventDate: "Must be ≥ 10 days out (2 days for sack lunches)",
        eventTime: "Service start time",
        students: "Expected student headcount",
        staff: "Expected staff/guest headcount",
        location: "Building + room #",
        theme: "Event purpose (e.g., HEALs, cultural meal)",
        food: "List items and quantities needed",
        drinks: "Include portion counts",
        other: "Supplies/equipment needed",
    };
    function setV(k: string, v: string) {
        update((d: any) => ({ ...d, vals: { ...d.vals, [k]: v } }));
    }
        ...
        <div className="fade-in">
            <div className="page-head">
                <div>
                    <h2>Food Request</h2>
                    <div className="ph-sub">
                        Dining Hall / Culinary Arts support request
                    </div>
                </div>
            </div>
            <div className="banner info" style={{ marginBottom: 14 }}>
                {I.alert()}
                <span>
                    Submit at least <b>ten (10) days</b> before the event (two
                    days for off-Center sack lunches) due to requisition &amp;
                            ...
                            >
                                ...
                                {f.type === "textarea" ? (
                                    <textarea
                                        ...
                                        value={vals[f.k] || ""}
                                        disabled={!canEdit}
                                        placeholder={FIELD_HINTS[f.k] || ""}
                                        onChange={(e) =>
                                            setV(f.k, e.target.value)
                                        }
                                    ></textarea>
                                ) : (
                                    <input
            ...
            <div className="card" style={{ marginBottom: 14 }}>
                ...
                <div className="card-body approvals">
                    <div className="appr-slot">
                        <span className="appr-line"></span>
                        <span className="appr-lbl">
                            Department Manager / Director
                        </span>
                    </div>
                    <div className="appr-slot">
                        <span className="appr-line"></span>
                        <span className="appr-lbl">F&amp;A Director</span>
                    </div>
                </div>
            </div>
            <div className="formbar">
