    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-body" style={{ padding: 24, textAlign: 'center' }}>
          <p style={{ margin: '0 0 18px', fontSize: 13.5, lineHeight: 1.5 }}>{message}</p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button className="btn" onClick={onCancel}>Cancel</button>
            <button className="btn primary" onClick={onConfirm} style={{ background: 'var(--red)', borderColor: 'var(--red)' }}>
              {I.check({ style: { width: 14, height: 14 } })} Confirm
            </button>
          </div>
        </div>
      </div>
...
export function SourceControl({
  ...
  const [confirm, setConfirm] = useState<{ action: string; entries: StagingEntry[] } | null>(null);
  ...
  const myStaged = staged.filter(s =>
    s.submitted_by === user.username || s.submitter_name?.startsWith(user.display_name)
  );
    ...
    try {
      ...
      const commit = await api.approveCommit({
        ...
        message: entries.length === 1
          ? (entries[0].operation ? OP_LABEL[entries[0].operation] || entries[0].change_type : entries[0].new_value_text || entries[0].field_name)
      ...
      <div className="page-head">
        ...
        <div className="ph-actions">
          {canReview && selected.size > 0 && (
            <button className="btn primary" onClick={() => setConfirm({ action: 'approveSelected', entries: staged.filter(s => selected.has(s.entry_id)) })} disabled={busy}>
              {I.check({ style: { width: 14, height: 14 } })} Commit selected ({selected.size})
            </button>
          )}
          {canReview && visibleStaged.length > 0 && (
            <button className="btn primary" onClick={() => setConfirm({ action: 'approveAll', entries: visibleStaged })} disabled={busy}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card">
            <div className="card-head">
              <h3>
                {canReview && visibleStaged.length > 0 && (
                  <label style={{ marginRight: 8, cursor: 'pointer' }} title="Select all">
                    <input
                      type="checkbox"
                ...
                {isStaff ? 'My pending submissions' : 'Review queue'}
              </h3>
              <span className="ch-link">{visibleStaged.length} staged</span>
            </div>
            <div className="card-body flush">
              {loading ? (
                <div style={{ padding: '26px 17px', textAlign: 'center', color: 'var(--faint)', fontSize: 12.5 }}>Loading\u2026</div>
                ...
                visibleStaged.map((ch) => {
                  ...
                  const icon = op ? OP_ICON[op] || 'clock' : 'clock';
                  const opLabel = op ? OP_LABEL[op] || op : ch.change_type;
                  const summary = opPayloadSummary(ch) || ch.new_value_text || ch.field_name;
                  return (
                    <div className="stage-item" key={ch.entry_id}>
                      {canReview && (
                        <label className="stage-cb" onClick={e => e.stopPropagation()}>
                          <input type="checkbox" checked={selected.has(ch.entry_id)} onChange={() => toggleSelect(ch.entry_id)} />
                        </label>
                      )}
                      <div className="stage-ic">{I[icon]({ style: { width: 15, height: 15 } })}</div>
                      <div className="stage-body">
                        <div className="stage-top">
                          <span className="stage-type">{opLabel}</span>
                          <span className="stage-items">{relTime(ch.created_at)}</span>
                        </div>
                        <div className="stage-summary">{summary}</div>
                        <div className="stage-meta">
                          ...
                          <b>{ch.submitter_name || ch.submitted_by}</b>
                          {ch.submitter_role && (
                            <span className={'pill role-' + ch.submitter_role}>
                      ...
                      {canReview ? (
                        <div className="stage-actions">
                          <button className="btn" style={{ padding: '6px 10px' }} onClick={() => handleReject(ch)} disabled={busy} title="Return to author">
                            {I.x({ style: { width: 14, height: 14 } })}
                          </button>
                          <button className="btn primary" style={{ padding: '6px 11px' }} onClick={() => setConfirm({ action: 'approve', entries: [ch] })} disabled={busy}>
        ...
        <div className="card" style={{ height: 'fit-content' }}>
          ...
          <div className="card-body flush">
            {commits.length === 0 && !loading ? (
              <div style={{ padding: '26px 17px', textAlign: 'center', color: 'var(--faint)', fontSize: 12.5 }}>No commits yet.</div>
            ) : (
              commits.map((c, i) => (
                <div className="commit-item" key={c.commit_id}>
                  ...
                  <div className="commit-body">
                    ...
                    <div className="commit-meta">
                      <span className="commit-hash">{shortSha(c.github_sha) || c.commit_id.slice(0, 7)}</span>
                      ...
                      {c.github_sha && <span className="synced-tag">{I.check({ style: { width: 11, height: 11 } })} synced</span>}
                    </div>
                    <div className="commit-diff">
                      <span className="diff-files">{c.change_count} field{c.change_count !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
                </div>
              ))
