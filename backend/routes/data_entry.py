...
def _first_admin() -> str:
    r = _client().table('user_profiles').select('id').eq('role', 'admin').limit(1).execute()
    if r.data:
        return r.data[0]['id']
...
def _stage_entries(ops: list[dict], batch_id: str, file_ref: str, submitter: str) -> list[dict]:
    ...
    for op_item in ops:
        ...
        if operation == 'inventory_save':
            ...
            summary = f"Menu: {payload.get('day', '')} — {len(payload.get('data', {}))} meals"
            entity_type = 'menu'
...
def _extract_ops(
    ...
    if operation == 'inventory_save':
        if rows is not None:
            ...
            result = mapper.map_rows_to_inventory(rows, categories, month, year, f'Imported from {filename}')
            if result is None:
                # deterministic failed — fall back to AI
                vendors = ctx.get_vendors()
                result = mapper.ai_extract_inventory(rows, categories, vendors, month, year, ai_config)
        else:
            categories = ctx.get_categories()
            vendors = ctx.get_vendors()
            result = mapper.ai_extract_inventory(text, categories, vendors, month, year, ai_config)
...
async def upload_file(
    ...
    try:
        ops = _extract_ops(file.filename or 'upload', content, hint, month, year, ai_config)
    except Exception as e:
        raise HTTPException(status_code=422, detail=f'Extraction failed: {e}')

    if not ops:
        raise HTTPException(status_code=422, detail='No data could be extracted from this file.')
...
async def preview_batch(batch_id: str):
    ...
    r = (
        ...
        .select('entry_id,operation,full_payload,entity_type,metadata,status,file_ref,created_at')
        .eq('batch_id', batch_id)
        .execute()
    )
    entries = r.data or []
    if not entries:
        raise HTTPException(status_code=404, detail='Batch not found or already committed.')

    pending = [e for e in entries if e.get('status') == 'pending']
    if not pending:
        raise HTTPException(status_code=410, detail='All entries in this batch have already been processed.')
    ...
    total_new = sum(
        sum(1 for row in d.get('rows', []) if row.get('status') == 'new')
        for d in diffs
    )
    total_update = sum(
...
@router.put('/settings')
async def update_settings(body: AISettingsBody):
