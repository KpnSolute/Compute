...
DEFAULT_CONFIG: dict = {
    ...
    "allowed_tools": [
        ...
        "get_reorders",
        "get_period_status",
        "stage_inventory_save",
        "stage_inventory_week_update",
...
def _check_rate_limit(user_id: str, user_role: str, cfg: dict) -> tuple[int, int]:
    ...
    r_h = (
        supabase_service.table("agent_usage")
    ...
    if used_h >= lim_h:
        raise HTTPException(
            status_code=429,
            detail=f"Rate limit: {lim_h} requests/hour reached. Try again later.",
        )
    ...
    if used_d >= lim_d:
        raise HTTPException(
            status_code=429,
            detail=f"Rate limit: {lim_d} requests/day reached. Try again tomorrow.",
        )
...
async def agent_chat(body: ChatRequest, authorization: str = Header("")):
    ...
    try:
        ...
        if ROLE_LEVEL.get(user_role, 0) < ROLE_LEVEL.get(min_role, 10):
            raise HTTPException(
                status_code=403, detail=f"Agent requires {min_role} role or above."
            )
        ...
        for _iteration in range(MAX_ITERATIONS):
            ...
            for tc in tool_calls:
                ...
                if tool_name not in role_tools:
                    result = {
                        "error": f"Tool '{tool_name}' is not available for your role or is disabled."
                    }
                    ...
                    try:
                        ...
                        result = TOOL_REGISTRY[tool_name](args_with_context, user_role)
...
def _summarize_result(tool_name: str, result: dict) -> str:
    ...
    summaries = {
        "get_dashboard_stats": lambda r: (
            f"{r.get('inventory_items', 0)} items, ${r.get('estimated_inventory_value', 0):,.2f} value, {r.get('upcoming_events', 0)} events, {r.get('items_below_par', 0)} below par"
        ),
        "get_inventory": lambda r: (
            f"{r.get('total_items', 0)} items, ${r.get('total_value', 0):,.2f} total, {r.get('below_par_count', 0)} below par"
        ),
        "get_events": lambda r: (
            f"{r.get('total', 0)} events, {len(r.get('upcoming', []))} upcoming"
        ),
        "get_menu": lambda r: (
            f"Menu for {r.get('day', '')} — {len(r.get('menu', {}))} meal periods"
        ),
        "get_reorders": lambda r: f"{r.get('reorder_count', 0)} items need reordering",
        "get_period_status": lambda r: (
            f"Period: {r.get('period_label', '')}, data present: {r.get('has_inventory_for_current_period', False)}"
        ),
        "get_users": lambda r: (
            f"{r.get('total_active', 0)} active users across {len(r.get('by_role', {}))} roles"
        ),
        "get_haccp_logs": lambda r: (
            f"{r.get('count', 0)} logs, {r.get('failures', 0)} failures"
        ),
        "get_daily_logs": lambda r: f"{r.get('count', 0)} log entries",
        "create_event": lambda r: (
            f"Event created: {r.get('event', {}).get('title', '')}"
        ),
        "get_ai_usage": lambda r: (
            f"{r.get('total_calls', 0)} calls, {r.get('total_tokens', 0)} tokens, ${r.get('total_cost', 0):.4f}"
        ),
    }
...
async def save_automations(body: AutomationsRequest, authorization: str = Header("")):
    ...
    try:
        ...
        supabase_service.table("app_settings").upsert(
            {
                "setting_key": key,
                "setting_value": body.automations,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save automations: {exc}"
        )
