...
async def list_sku_review(
    ...
    try:
        q = (
            supabase_service.table("sku_review_queue")
            .select(
...
async def resolve_sku(
    ...
    if body.resolution == "new_item":
        ...
        _apply_entries(
            ...
            message=f"SKU review: new item {sku}",
            source="sku_review",
        )
    ...
    # ── alias_existing: metadata-only write via RPC (no inventory change) ─────
        ...
        if not body.item_id:
            raise HTTPException(
                status_code=422, detail="item_id required for alias_existing"
            )
        try:
            alias_res = supabase_service.rpc(
                "sku_add_alias",
                {
                    "p_item": body.item_id,
                    "p_alias": parsed_sku,
            ...
            if not alias_data.get("ok"):
                raise HTTPException(
                    status_code=409, detail=alias_data.get("error", "Alias collision")
                )
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"sku_add_alias failed: {str(e)}"
            )
    ...
    # ── override_existing: rename canonical SKU via staging → commit ──────────
        ...
        if not body.item_id:
            raise HTTPException(
                status_code=422, detail="item_id required for override_existing"
            )
