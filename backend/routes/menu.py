...
@router.post('/{day}')
async def update_menu(day: str, body: MenuUpdate, auth_user: dict = Depends(_get_auth_user)):
    if day not in VALID_DAYS:
        raise HTTPException(status_code=400, detail=f'Invalid day: {day}')
    ...
    supabase.table('menu_entries').delete().eq('day_of_week', day).eq('cycle_id', cycle_id).execute()
