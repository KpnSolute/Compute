-- 018_inventory_audit_log.sql
-- Phase 2b: post-session inventory auditor. A deterministic period audit writes
-- "logical issue" findings to an in-app table surfaced in Data Entry. Checks:
--   negative_ending      — ending = opening + received - issued < 0 (over-pulled)
--   reconciliation_drift — cached w{n} column != ledger SUM (data integrity)
--   missing_price        — item has activity but unit_price = 0
--   orphan_item          — item has activity but no category (never categorized)
--   suspicious_qty       — a single ledger movement is abnormally large
--   duplicate_week       — same item received many times in one week (possible double-entry)
-- Idempotent: re-running a period clears its prior UNRESOLVED findings first.
-- Safe + repeatable (IF NOT EXISTS / CREATE OR REPLACE).
...
CREATE POLICY inv_audit_service_all ON public.inventory_audit_log
...
DECLARE
    v_susp_qty numeric := 500;   -- single-movement "abnormally large" threshold
    v_count integer := 0;
BEGIN
    -- idempotent: clear prior UNRESOLVED findings for this period
    DELETE FROM inventory_audit_log WHERE month=p_month AND year=p_year AND resolved=false;

    -- 1) negative / over-pulled ending
    ...
    -- 2) reconciliation drift: cached column != ledger SUM (per item/week/direction)
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
    ...
    -- 3a) missing price: item has activity but unit_price = 0
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
    ...
    -- 3b) orphan item: activity but no category
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
    ...
    -- 4a) suspicious single movement (abnormally large quantity)
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
    ...
    -- 4b) possible duplicate: same item received 3+ times in one week
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
