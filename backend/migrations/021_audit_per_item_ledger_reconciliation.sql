-- 021_audit_per_item_ledger_reconciliation.sql
-- Full-month spreadsheet imports now write week 1-4 inventory_transactions for
-- any weekly received/issued cells they contain. Reconciliation should therefore
-- compare cached monthly_inventory week columns against the ledger at item +
-- direction granularity, not use a broad period-level gate.
--
-- This keeps legacy no-ledger imports quiet, but catches real drift for every
-- item/direction that has week 1-4 ledger evidence.
...
BEGIN
    ...
    -- reconciliation_drift: compare only item/direction pairs that have week
    -- 1-4 ledger rows. Week 0 aggregate rows remain excluded because they are
    -- month-level pull totals and do not populate weekly cache columns.
    INSERT INTO inventory_audit_log (month,year,check_type,severity,item_id,sku,message,details)
    SELECT p_month,p_year,'reconciliation_drift','error',mi.item_id,ii.sku,
        ...
        jsonb_build_object('cached_received',
            ...
            'checked_received', COALESCE(lr.has_recv,false),
            'checked_issued', COALESCE(lr.has_iss,false))
    FROM monthly_inventory mi JOIN inventory_items ii ON ii.id=mi.item_id
    JOIN (
        SELECT item_id,
               ...
               COUNT(*) FILTER (WHERE txn_type IN ('received','adjustment_increase')) > 0 has_recv,
               COUNT(*) FILTER (WHERE txn_type IN ('issued','adjustment_decrease')) > 0 has_iss
    ...
    WHERE mi.month=p_month AND mi.year=p_year
      AND (
        (lr.has_recv AND (mi.w1_received+mi.w2_received+mi.w3_received+mi.w4_received) <> COALESCE(lr.recv,0))
        OR (lr.has_iss AND (mi.w1_issued+mi.w2_issued+mi.w3_issued+mi.w4_issued) <> COALESCE(lr.iss,0))
      );
